...
def get_token_auth_header():
    ...
    if 'Authorization' not in request.headers:
            raise AuthError({'code': 'invalid_header',
                                'description': 'Authorization header need to be present'}, 401)
    authorization = request.headers.get('Authorization')     
    splitted_authorization_parts = authorization.split(' ')
    if len(splitted_authorization_parts) != 2:
                raise AuthError({'code': 'invalid_header',
                                'description': 'Authorization header need to be in Bearer token'}, 401)
    if splitted_authorization_parts[0].lower() != 'bearer':
                raise AuthError({'code': 'invalid_header',
                                'description': 'Authorization header need to be to start with Bearer'}, 401)
